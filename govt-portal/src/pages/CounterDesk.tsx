...
export const CounterDesk: React.FC = () => {
  ...
  const handleQuickAadhaarLookup = async () => {
    try {
      const resp = await fetch('http://localhost:8000/api/v1/triage/auth/aadhaar/verify-otp', {
        method: 'POST',
  ...
  const handleAIAnalyze = async () => {
    setEvaluating(true);
    try {
      const resp = await fetch('http://localhost:8000/api/v1/triage/evaluate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          symptom_ids: [],
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Instant Aadhaar e-KYC lookup, AI symptom specialty matcher, and thermal token generation for walk-in citizens.
          </p>
        </div>
          ...
          <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>2. Chief Complaint & AI Specialty Matcher</span>
              </span>
              <button
                type="button"
                disabled={evaluating}
