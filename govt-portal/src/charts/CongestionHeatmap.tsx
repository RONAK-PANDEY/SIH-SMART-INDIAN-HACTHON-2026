      ...
      {facilities.map((f) => (
        ...
        >
          <div>
            <div className="flex justify-between items-start">
              ...
              <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
                f.status === 'OVERLOAD' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : f.status === 'MODERATE' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
              }`}>
                {f.load}% Load
              </span>
            </div>
          ...
          <div className="mt-4 pt-2 border-t border-slate-800 flex justify-between items-center text-xs">
            ...
            <strong className="text-sm font-black text-white">{f.wait}</strong>
          </div>
        </div>
      ))}
    </div>
