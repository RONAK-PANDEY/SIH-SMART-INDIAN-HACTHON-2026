...
def test_staffing_deficit_zero_when_overstaffed():
    ...
    score, detail, _, available = compute_staffing_subscore(inp)
...
# ---------------------------------------------------------------------------
# Missing-data handling (regression tests for the graceful-degradation fix)
# ---------------------------------------------------------------------------
def test_missing_staffing_and_wait_data_is_excluded_not_fabricated():
    """Previously: omitting doctors_on_duty/doctors_required/avg_wait_time_minutes
    silently defaulted to 0, which read as 'zero doctors on duty' (max staffing
    alarm) and 'zero-minute wait' (best-case wait reading) -- both fabricated
    from absent data. Now these drivers must be excluded from the score."""
    payload = {
        "department": "Radiology",
        "timestamp": "2026-09-02T03:00:00Z",
        "current_queue_length": 5,
        "historical_avg_queue_length": 5,
        "patient_arrivals_last_hour": 3,
        "historical_avg_arrivals_last_hour": 3,
        # doctors_on_duty, doctors_required, discharges_last_hour,
        # avg_wait_time_minutes deliberately omitted.
    }
    inp = CongestionInput.from_dict(payload)
    rule_score, sub_scores, _ = compute_rule_score(inp)

    by_name = {s.name: s for s in sub_scores}
    assert by_name["staffing"].available is False
    assert by_name["wait_time"].available is False
    assert "not provided" in by_name["staffing"].detail.lower()
    assert "not provided" in by_name["wait_time"].detail.lower()

    # Only queue (calm) and flow (calm, arrivals-only) drove the score --
    # both are near-zero pressure, so the score should be near-zero too,
    # not the ~32 a fabricated "zero doctors on duty" reading would produce.
    assert rule_score < 10

    reason = build_reason(sub_scores)
    assert "staffing" in reason.lower()
    assert "wait_time" in reason.lower() or "wait time" in reason.lower()
    # Must not claim a wait time reading that was never reported.
    assert "waiting time 0 min" not in reason.lower()


def test_missing_discharges_does_not_maximize_flow_imbalance():
    """Previously: omitting discharges_last_hour defaulted it to 0, which
    (via the discharges>0 branch) forced ratio=2.0 -- the exact saturation
    point for 'maximum flow imbalance' -- from data that was never sent."""
    from backend.ml.congestion_prediction.features import compute_flow_subscore
    inp = calm_input(discharges_last_hour=None)
    score, detail, ratio, available = compute_flow_subscore(inp)
    assert available is True  # arrivals data alone is still usable
    assert ratio is None
    assert score < 10  # arrivals are calm in calm_input(); no fabricated imbalance
    assert "not provided" in detail.lower()


def test_full_service_end_to_end_surfaces_missing_inputs():
    payload = {
        "department": "Radiology",
        "timestamp": "2026-09-02T03:00:00Z",
        "current_queue_length": 5,
    ...
    assert set(result["missing_inputs"]) == {"staffing", "wait_time"}
